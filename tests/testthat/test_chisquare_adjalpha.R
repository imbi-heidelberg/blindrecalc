...
test_that("adjusted_alpha gives actual level at most nominal level", {
  ...
  alpha_max2 <- max(toer(design1, n1 = n1_new, nuisance = nuis_vec,
                         recalculation = FALSE, allocation = "approximate"))


  design2 <- setupChiSquare(alpha = 0.025, beta = 0.2, r = 3, delta = 0.3)
  ...
  alpha_max3 <- max(toer(design2, n1 = 50, nuisance = nuis_vec,
                         recalculation = TRUE, allocation = "approximate"))
